...
struct Texture 
{
	...
	void load_p(const char* texture_path)
	{
		GLuint texture_;

		int width, height, comp;
		unsigned char* pimage0 = stbi_load(texture_path, &width, &height, &comp, 2);
	...
	void load_mip(const char* texture_path) 
	{
		GLuint texture_;

		int width, height, comp;
		unsigned char* pimage0 = stbi_load(texture_path, &width, &height, &comp, 3); /* convert 1-channel to 3-channel image */
		int stride0 = width * comp, stride1 = (stride0 + 4) & (~4);	// 4-byte aligned stride
		unsigned char* pimage = (unsigned char*)malloc(sizeof(unsigned char) * stride1 * height);
		for (int y = 0; y < height; y++) memcpy(pimage + (height - 1 - y) * stride1, pimage0 + y * stride0, stride0); // vertical flip
		...
		int mip_levels = miplevels(2048, 1024);
		for (int k = 1, w = width >> 1, h = height >> 1; k < mip_levels; k++, w = max(1, w >> 1), h = max(1, h >> 1))
