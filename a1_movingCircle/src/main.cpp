...
static const float  MIN_SCALE = 30.0f;
static const float  MAX_SCALE = 300.0f;
...
void print_help()
{
	...
	printf("- press 'p/m' to increase/decrease the speed scales (min=%d, max=%d, +-10)\n", (uint)MIN_SCALE, (uint)MAX_SCALE);
...
void keyboard(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS)
	{
		...
		else if (key == GLFW_KEY_P)
		{
			for (auto& c : circles)
			{
				if (c.speed_scale >= MAX_SCALE) return;
				c.speed_scale += 10.0f;
			}
			printf("speed_scale:% -4f\r", circles[0].speed_scale);
		}
		else if (key == GLFW_KEY_M)
		{
			for (auto& c : circles)
			{
				if (c.speed_scale <= MIN_SCALE) return;
				c.speed_scale -= 10.0f;
			}
			printf("speed_scale:% -4f\r", circles[0].speed_scale);
		}
